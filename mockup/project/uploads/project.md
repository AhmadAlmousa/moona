...
- Let two users collaborate on the same list in near realtime.
...
Product names are shared across all users and power autocomplete. User lists should reference product IDs so names can be normalized and reused.
...
- Existing users can sign in with their saved password.
- Unknown user IDs create a new account automatically.
- New users default to Arabic language, dark theme, no sharing, and an empty owned list.
...
- Show category badge in the all-items view when present.
...
- Long press on touch devices opens edit.
...
4. A five-second timer starts.
5. If Undo is selected, the timer is canceled and the card returns to normal.
6. If the timer expires, the item is removed from the list.
...
Sharing is one-to-one and owner-based.
...
2. User enters another existing user ID.
3. The app validates both users exist.
4. The app rejects sharing with oneself.
5. The target user is linked to the sharer's list.
...
- A user can share with only one other user at a time.
- A user can receive only one shared list at a time.
...
The admin area manages global configuration and user records.
...
- User-facing operations should verify that the caller is allowed to act for the requested user.
- Automatic account creation is convenient but may allow unwanted accounts.
- Sharing is currently intentionally one-to-one.
- Marking an item off currently deletes it after an undo window; there is no completed-items history.
